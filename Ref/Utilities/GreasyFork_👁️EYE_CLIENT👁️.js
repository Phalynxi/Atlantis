...
// @namespace    none
// @version      1.3
// @description  MOOMOOD.         PRESS V=SPIKE/  PRESS F=TRAP/      AFTO HAT   AFTO PUNCH
// @author       20 FPS xVIPx
// @match        *://moomoo.io/*
...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/483441/%F0%9F%91%81%EF%B8%8FEYE%20CLIENT%F0%9F%91%81%EF%B8%8F.user.js
// @updateURL https://update.greasyfork.org/scripts/483441/%F0%9F%91%81%EF%B8%8FEYE%20CLIENT%F0%9F%91%81%EF%B8%8F.meta.js
// ==/UserScript==
...
document.getElementById('gameName').innerHTML = '👁️EYE CLIENT👁️';
document.getElementById('loadingText').innerHTML = '....LOADING....'
document.getElementById('diedText').innerHTML = "R.I.P";
document.getElementById('diedText').style.color = "#ffffff";
document.title = ' vnmod';
document.getElementById("leaderboard").append ('🔥FI☄️RE🔥');
...
setInterval(function() {
    if (myPlayer.hat == 45) {
        doNewSend(["6", ["PROJECT INVINSIBLE!"]]);
    }
}, 100);
function normal() {
    hat(normalHat);
...
function handleMessage(m){
    ...
    if(item == "O" && data[1] == myPlayer.id) {
        ...
        if (data[2] < 33 && data[2] > 0) {//antiinsta no sold
        ...
        if (data[2] < 51 && data[2] > 40) {//antiinsta for pol
        ...
        if (data[2] < 56 && data[2] > 50) {//bullspam heal
