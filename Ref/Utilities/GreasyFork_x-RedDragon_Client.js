...
// @version      1.6.6.5
// @description  Hotkeys & Features: R=Insta | T=ReverseInsta | Y=BoostInsta | G=BoostSpike | B=4xTraps/Boost | C=4xSpikes | M=AutoMills | F=Trap/BoostPad | V=Spike | N=Mill | H=Teleport/Turret | AutoHat | Esc=Visuals Menu & Music | RightClick=FastBreak | AutoHeal | AutoGG | AntiInstas | Visual Enhancements
    ...
    if (data[0] === "a") {
        ...
        for (let i = 0; i < data[1].length / 13; i++) {
            ...
            if (obj[0] === myPlayer.id) {
              ...
              myPlayer.y = obj[2];
              myPlayer.dir = obj[3];
              myPlayer.object = obj[4];
              myPlayer.weapon = obj[5];
              myPlayer.clan = obj[7];
              myPlayer.isLeader = obj[8];
              myPlayer.hat = obj[9];
              myPlayer.accessory = obj[10];
              myPlayer.isSkull = obj[11];
            } else {
                const isAlly =
                    myPlayer.clan &&
                    obj[7] &&
        ...
        if (enemiesNear.length > 0) {
            ...
            for (let enemy of enemiesNear) {
                let dx = enemy[1] - myPlayer.x;
                let dy = enemy[2] - myPlayer.y;
...
let biomeLoopActive = false;
let enemyInside = false;
let resumeTimeout = null;

const BIOME_ACCESSORY = 11;
...
function getBiomeHat() {
  if (!myPlayer || typeof myPlayer.y !== "number") return null;
  if (myPlayer.y < 2400) return 15;
  if (myPlayer.y > 6850 && myPlayer.y < 7550) return 31;
  return 12;
}

function biomeLoopCycle() {
  if (!biomeLoopActive) return;

  if (enemyInside) {
    setTimeout(biomeLoopCycle, 800);
    return;
  }

  const biomeHat = getBiomeHat();
  if (biomeHat !== null) doNewSend(["c", [0, biomeHat, 0]]);
  equipAccessory(BIOME_ACCESSORY);

  setTimeout(biomeLoopCycle, 800);
}

function startBiomeLoop() {
  if (biomeLoopActive) return;
  if (!myPlayer || myPlayer.x == null) return;

  biomeLoopActive = true;
  biomeLoopCycle();
}

function stopBiomeLoop() {
  biomeLoopActive = false;
}
...
function exitCombatMode() {
  enemyInside = false;
  if (!rightClickActive) startBiomeLoop();
}
...
setInterval(() => {
  ...
  const dx = myPlayer.x - nearestEnemy[1];
  const dy = myPlayer.y - nearestEnemy[2];
  const inside = Math.sqrt(dx * dx + dy * dy) <= 400;

  if (inside && !enemyInside) enterCombatMode();
  if (!inside && enemyInside) exitCombatMode();
}, 50);
...
document.addEventListener("mousedown", e => {
  ...
  if (e.button === 2) {
    ...
    if (rightClickActive) stopBiomeLoop();
    else if (!enemyInside) startBiomeLoop();
  }
});
...
function rd_cycle() {
  if (!rd_loop) return;
  if (rd_enemy || rd_keys.size > 0) {
    setTimeout(rd_cycle, 800);
    return;
  }

  const hat = rd_getHat();
  if (hat !== null) doNewSend(["c", [0, hat, 0]]);

  setTimeout(() => {
    if (!rd_loop || rd_enemy || rd_keys.size > 0) return;
    storeEquip(11, 1);
  }, 500);

  setTimeout(rd_cycle, 1000);
}
...
setInterval(() => {
  const inside = rd_enemyRange();
  if (inside) {
    if (!rd_enemy) doNewSend(["c", [0, 6, 0]]);
    rd_enemy = true;
    rd_stop();
  } else {
    rd_enemy = false;
...
function rd_tryResume() {
  if (rd_resume) clearTimeout(rd_resume);
  rd_resume = setTimeout(() => {
    if (rd_keys.size === 0 && !rd_enemy) rd_start();
  }, 700);
}

const rd_block = ["KeyR","KeyT","KeyY","KeyU","Space"];

document.addEventListener("keydown", e => {
  if (rd_block.includes(e.code)) {
    rd_keys.add(e.code);
    rd_stop();
  }
});
...
const rd_wait = setInterval(() => {
  if (myPlayer && myPlayer.x != null) {
    clearInterval(rd_wait);
    rd_start();
  }
}, 300);
})();
...
(function() {
    ...
    function isGridEnabled() {
        return window.RD_VISUALS?.grid === true;
    }
...
(function () {
	...
	window.Cow.addRender("global-reload-bars", () => {

		if (!window.RD_VISUALS?.reloadBars) return;
...
(function () {
    ...
    if (!window.RD_VISUALS) {
        window.RD_VISUALS = {};
    }

    if (!window.RD_VISUALS.hpBuild) {
        window.RD_VISUALS.hpBuild = {
            enabled: true,
        ...
        if (!window.RD_VISUALS.hpBuild.colorMine) window.RD_VISUALS.hpBuild.colorMine = "#6aff4f";
        if (!window.RD_VISUALS.hpBuild.colorAlly) window.RD_VISUALS.hpBuild.colorAlly = "#fcec5f";
        if (!window.RD_VISUALS.hpBuild.colorEnemy) window.RD_VISUALS.hpBuild.colorEnemy = "#ff4f4f";
    }
                ...
                try {
                    ...
                    switch(opcode){
                        case "C":
                            ...
                            break;
                        case "3":
                            if(data[0]==null) teammates = [];
                            break;
                        case "4":
                            teammates = [];
                            for(let i=0;i<data[0].length;i+=2){
                                teammates.push(data[0][i]);
                            }
                            break;
                    }
                } catch(e){}
    ...
    function getObjectType(o){
        if(!o.owner || !o.owner.sid) return "enemy";
        if(o.owner.sid === mySID) return "my";
        if(teammates.includes(o.owner.sid)) return "ally";
        return "enemy";
    }

    function drawHP(ctx, o){
        if(!window.RD_VISUALS?.hpBuild?.enabled) return;
        const max = o.maxHealth || o.maxHP || 100;
        const cur = o.health || o.hp || max;
        if(window.RD_VISUALS.hpBuild.onlyWhenDamaged && cur>=max) return;
        const hp = cur/max;
        const r = 22;
        const w = 10;
        const a = hp*Math.PI*2;
        const type = getObjectType(o);
        let mainColor = window.RD_VISUALS.hpBuild.colorEnemy;
        let bgColor = "#2b0000";
        if(type==="my"){
            mainColor = window.RD_VISUALS.hpBuild.colorMine;
            bgColor = "#003300";
        } else if(type==="ally"){
            mainColor = window.RD_VISUALS.hpBuild.colorAlly;
            bgColor = "#4a4300";
        }
        ctx.save();
        ...
        ctx.arc(0,0,r,0,a);
        ctx.stroke();
        if(window.RD_VISUALS.hpBuild.showPercent){
    ...
    function init(){
        ...
        function render(){
            ...
            Cow.objectsManager.eachVisible((o)=>{
                if((o.health==null && o.hp==null) || (o.maxHealth==null && o.maxHP==null)) return;
                drawHP(ctx, o);
            });
        }
        if(Cow.addRender){
...
let selfId = null, container = null, hooked = false;

function getContainer(){
    if(container) return container;

    container = document.createElement("div");
    container.id = "xrd-radar-layer";
    ...
    if(menu){
        const z = parseInt(getComputedStyle(menu).zIndex);
        container.style.zIndex = !isNaN(z) ? z - 1 : 1;
    } else {
...
function hookSocket(){
    ...
    const original = WebSocket.prototype.addEventListener;
...
function getArrow(id){
    let el = document.getElementById("xrd-r-"+id);
    if(el) return el;
...
function loop(){
    ...
    if(!selfId || !radarMap.has(selfId)) return;

    if(!window.RD_VISUALS?.radar?.enabled){
        document.querySelectorAll('[id^="xrd-r-"]').forEach(e=>e.style.display="none");
        return;
    }

    const me = radarMap.get(selfId);
    const cx = innerWidth/2;
    const cy = innerHeight/2;

    for(const [id,p] of radarMap){

        if(id === selfId) continue;

        const el = getArrow(id);
        ...
        const enemyColor = window.RD_VISUALS.radar.enemyColor;
        const teamColor = window.RD_VISUALS.radar.teamColor;

        Object.assign(el.style,{
            display:"block",
            opacity:dist,
            transform:"rotate("+rot+"deg)",
            left:(cx+Math.cos(ang)*cy*dist)+"px",
            top:(cy-Math.sin(ang)*cy*dist)+"px",
            borderColor: p.t && p.t===me.t
                ? "transparent transparent transparent "+teamColor
                : "transparent transparent transparent "+enemyColor
        });
    }

    document.querySelectorAll('[id^="xrd-r-"]').forEach(e=>{
        const id = +e.id.replace("xrd-r-","");
        if(!radarMap.has(id)) e.remove();
    });
}
...
:root{
--bg-main:#0a0a0a;--bg-panel:#1a1a1a;
--accent:#c40000;--accent-dark:#550000;
--radius-lg:16px;--radius-md:12px;--radius-sm:8px
}
#moddedMenu{position:fixed;inset:0;display:none;justify-content:center;align-items:center;background:rgba(0,0,0,.6);backdrop-filter:blur(6px);z-index:999999}
#moddedMenu.open{display:flex}
.rd-wrapper{width:900px;height:560px;background:var(--bg-main);border-radius:var(--radius-lg);padding:14px;display:flex;flex-direction:column;gap:12px;box-shadow:0 0 40px rgba(196,0,0,.7)}
.rd-header{height:56px;background:var(--bg-panel);border-radius:var(--radius-md);padding:0 18px;display:flex;align-items:center;position:relative;gap:12px}
.rd-header h1{font-size:18px;font-weight:600;color:#fff;margin:0;display:flex;align-items:center;gap:8px}
...
.rd-nav-btn{height:46px;border-radius:var(--radius-sm);border:none;background:var(--bg-main);color:#fff;cursor:pointer;transition:.25s;font-size:14px}
...
.rd-page{position:absolute;inset:16px;opacity:0;transform:translateY(20px);pointer-events:none;transition:.3s;overflow:auto}
...
<div class="rd-wrapper">
 <div class="rd-header">
  <h1><img src="https://i.imgur.com/vXgDJSp.png" width="40"> 𝕩-ℝ𝕖𝕕𝔻𝕣𝕒𝕘𝕠𝕟 ℂ𝕝𝕚𝕖𝕟𝕥</h1>
  <button class="rd-close">x</button>
 </div>
 <div class="rd-main">
  <div class="rd-navbar">
   <button class="rd-nav-btn active" data-id="info">Info</button>
   <button class="rd-nav-btn" data-id="music">Music</button>
  </div>
  <div class="rd-content">
   <div class="rd-page active" data-page="info" id="infoPage">
     <h2>Info</h2>
     <p style="
...
">
=====================================================
x-RedDragon Client – UserScript Documentation
=====================================================

Version: 1.6.6.5
Author: x-RedDragon
Description: Hotkeys and advanced features for moomoo.io.
Includes visual enhancements, autoaim, autoheal, automatic structure placement, radar, improved menus, HP display options, and more.
License: MIT

Important Links:
• YouTube: https://www.youtube.com/@x-RedDragonOficial
• Download: https://update.greasyfork.org/scripts/543548/x-RedDragon%20Client.user.js
• Update: https://update.greasyfork.org/scripts/543548/x-RedDragon%20Client.meta.js
...
• Custom UI & Menu
• FPS & Ping
• Reload & HP Bars (with "Show HP when" options: HP < Max / Always)
• Radar with team/enemy arrow colors behind main menu
• Name prefix: x-RedDragon
• Grid overlay
• Toggleable features for all visuals

Menu & UI Enhancements:
• Fully interactive Visuals page
• Toggles for radar, HP builds, grid, reload bars
• Color pickers for HP and radar arrows
• Dropdowns for display modes
• Smooth animations and flexible layout
</p>

    <button class="rd-button" id="ytBtn">Visit the YouTube channel</button>
   </div>
   <div class="rd-page" data-page="music" id="musicPage">
     <h2>Music</h2>
     <select class="rd-select" id="musicSelect"></select>
     <div id="buttons">
       <button class="rd-button" id="playBtn">Play</button>
       <button class="rd-button" id="stopBtn">Stop</button>
     </div>
     <select class="rd-select" id="modeSelect">
       <option value="repeat">Repeat</option>
       <option value="next">Next</option>
     </select>
     <audio id="audioPlayer"></audio>
   </div>
   <div class="rd-page" data-page="bot" id="botPage">
     <!-- Bot code here -->
   </div>
  </div>
 </div>
...
function toggle(label, value, cb) {
    ...
    sw.style.borderRadius = '20px';
    sw.style.background = value ? 'var(--accent)' : '#333';
    sw.style.cursor = 'pointer';
    sw.style.transition = '0.25s';
    ...
    slider.style.left = value ? '24px' : '2px';
    ...
    input.onchange = () => {
        sw.style.background = input.checked ? 'var(--accent)' : '#333';
        slider.style.left = input.checked ? '24px' : '2px';
        cb(input.checked);
    };
...
menu.querySelectorAll('.rd-nav-btn').forEach(btn=>{
    btn.onclick=()=>{
        ...
        menu.querySelectorAll('.rd-nav-btn,.rd-page').forEach(e=>e.classList.remove('active'));
        btn.classList.add('active');
        menu.querySelector(`[data-page="${id}"]`).classList.add('active');
    };
});
document.getElementById('ytBtn').addEventListener('click', () => {
    window.open('https://www.youtube.com/@x-RedDragonOficial','_blank');
});
...
const songs = [
  ...
  {name:"Rival - Lonely Way (Link Down)",url:null},
  {name:"Rival - Throne (Link Down)",url:null},
  {name:"Rival - Be Gone (Link Down)",url:null},
  {name:"Rival - Walls (Link Down)",url:null},
  {name:"Rival - Control (Link Down)",url:null},
  {name:"Egzod - No Rival (Link Down)",url:null},
  {name:"do not resurrect - Necrotic Grip (Link Down)",url:null},
  {name:"Witchouse 40k - Black Rainbow (Link Down)",url:null},
  {name:"Grim Salvo - Feasting (Link Down)",url:null},
  {name:"Initial D - Don't Stand so Close (Link Down)",url:null},
  {name:"Initial D - The Top (Link Down)",url:null},
  {name:"Initial D - Gas Gas Gas (Link Down)",url:null},
  {name:"Initial D - Running In The 90's (Link Down)",url:null},
  {name:"Initial D - No One Sleep In Tokyo (Link Down)",url:null},
  {name:"UNSECRET & Noeni - Fallout (Link Down)",url:null},
  {name:"V O E - Giants (Link Down)",url:null},
  {name:"Neoni - Champion (Link Down)",url:null},
  {name:"JPB & Mendum - Losing Control (Link Down)",url:null},
  {name:"Freddie Dredd - Limbo (Link Down)",url:null},
  {name:"xxxmanera - NFS (Link Down)",url:null},
  {name:"Take Me Home (Link Down)",url:null},
  {name:"rarin - GTA (Link Down)",url:null},
  {name:"Ghost (Link Down)",url:null},
  {name:"Rico Story 1 (Link Down)",url:null},
  {name:"xxxmanera - Loyalty Before Royalty (Link Down)",url:null},
  {name:"xxxmanera - Sosa! Baby (Link Down)",url:null},
  {name:"xxxmanera - Dead Idol (Link Down)",url:null},
  {name:"CRVN x Zack Merci - Nobody (Link Down)",url:null},
  {name:"Cheriimoya, Sierra Kidd - Living Life (Link Down)",url:null},
  {name:"Adrenaline - ACE (Link Down)",url:null}
];
...
playBtn.addEventListener('click', () => {
    ...
    if(audio.src !== musicSelect.value) audio.src = musicSelect.value;
    audio.play();
});
...
audio.addEventListener('ended', () => {
    if (modeSelect.value === 'repeat') {
        ...
        let idx = musicSelect.selectedIndex;
        const total = musicSelect.options.length;
        do {
            idx = (idx + 1) % total;
        } while (!musicSelect.options[idx].value);
        musicSelect.selectedIndex = idx;
        playMusic();
    }
});

(function initVisualsModule() {

function waitForMenu(){
    const menu = document.getElementById('moddedMenu');
    if(!menu){ setTimeout(waitForMenu,100); return; }

    if(!window.RD_VISUALS) window.RD_VISUALS = {};
    if(!window.RD_VISUALS.radar) window.RD_VISUALS.radar = { enabled:true, enemyColor:"#ff0000", teamColor:"#00ff00" };
    if(window.RD_VISUALS.grid==null) window.RD_VISUALS.grid = false;
    if(window.RD_VISUALS.reloadBars==null) window.RD_VISUALS.reloadBars = true;
    if(!window.RD_VISUALS.hpBuild) window.RD_VISUALS.hpBuild = {
        enabled:true,
        onlyWhenDamaged:true,
        showPercent:true,
        colorMine: "#6aff4f",
    ...
    const navBar = menu.querySelector('.rd-navbar');
    const content = menu.querySelector('.rd-content');

    const visualsBtn = document.createElement('button');
    visualsBtn.className = 'rd-nav-btn';
    visualsBtn.dataset.id = 'visuals';
    visualsBtn.textContent = 'Visuals';
    navBar.appendChild(visualsBtn);

    const visualsPage = document.createElement('div');
    visualsPage.className = 'rd-page';
    visualsPage.dataset.page = 'visuals';
    visualsPage.style.display="flex";
    visualsPage.style.flexDirection="column";
    visualsPage.style.gap="10px";
    visualsPage.style.padding="12px";
    content.appendChild(visualsPage);

    visualsBtn.onclick = ()=>{
        navBar.querySelectorAll('.rd-nav-btn').forEach(b=>b.classList.remove('active'));
        content.querySelectorAll('.rd-page').forEach(p=>p.classList.remove('active'));
        visualsBtn.classList.add('active');
        visualsPage.classList.add('active');
    };

    function createToggle(label,state,cb){
        const row=document.createElement("div");
        row.style.display="flex";
        row.style.justifyContent="space-between";
        row.style.alignItems="center";
        ...
        toggle.style.background=state?"var(--accent)":"#444";
    ...
    function createColor(label,value,cb){
        const row=document.createElement("div");
        row.style.display="flex";
        row.style.justifyContent="space-between";
        row.style.alignItems="center";
    ...
    function createSelect(label,options,current,cb){
        const row=document.createElement("div");
        row.style.display="flex";
        row.style.justifyContent="space-between";
        row.style.alignItems="center";
    ...
    radarContainer.style.display = window.RD_VISUALS.radar.enabled ? "flex" : "none";
    radarContainer.style.flexDirection = "column";
    radarContainer.style.gap = "10px";
    radarContainer.style.marginLeft = "14px";

    const radarToggle = createToggle(
        "Radar",
        window.RD_VISUALS.radar.enabled,
        v=>{
            window.RD_VISUALS.radar.enabled = v;
            radarContainer.style.display = v ? "flex" : "none";
        }
    );

    visualsPage.appendChild(radarToggle);
    visualsPage.appendChild(radarContainer);

    radarContainer.appendChild(
        createColor("Enemy Arrow Color",window.RD_VISUALS.radar.enemyColor,v=>{
            window.RD_VISUALS.radar.enemyColor=v;
        })
    );

    radarContainer.appendChild(
        createColor("Team Arrow Color",window.RD_VISUALS.radar.teamColor,v=>{
            window.RD_VISUALS.radar.teamColor=v;
        })
    );

    visualsPage.appendChild(createToggle("Show Grid", window.RD_VISUALS.grid, v=>{
        window.RD_VISUALS.grid=v;
    }));

    visualsPage.appendChild(createToggle("Reload Bars", window.RD_VISUALS.reloadBars, v=>{
        window.RD_VISUALS.reloadBars=v;
    }));

    const hpContainer = document.createElement("div");
    hpContainer.style.display = window.RD_VISUALS.hpBuild.enabled ? "flex" : "none";
    hpContainer.style.flexDirection = "column";
    hpContainer.style.gap = "10px";
    hpContainer.style.marginLeft = "14px";

    const hpToggle = createToggle(
        "HP Builds",
        window.RD_VISUALS.hpBuild.enabled,
        v=>{
            window.RD_VISUALS.hpBuild.enabled = v;
            hpContainer.style.display = v ? "flex" : "none";
        }
    );

    visualsPage.appendChild(hpToggle);
    visualsPage.appendChild(hpContainer);

    hpContainer.appendChild(
        createSelect(
            "Display HP when:",
            [
                {value:"damaged",label:"HP < Max"},
                {value:"always",label:"Always"}
            ],
            window.RD_VISUALS.hpBuild.onlyWhenDamaged ? "damaged" : "always",
            v=>{
                window.RD_VISUALS.hpBuild.onlyWhenDamaged = (v === "damaged");
            }
        )
    );
    ...
    hpContainer.appendChild(
        createToggle(
            "Show Percentage",
            window.RD_VISUALS.hpBuild.showPercent,
            v=>{
                window.RD_VISUALS.hpBuild.showPercent = v;
            }
        )
    );
    ...
    hpContainer.appendChild(
        createColor(
            "My Structures Color",
            window.RD_VISUALS.hpBuild.colorMine,
            v=>{
                window.RD_VISUALS.hpBuild.colorMine = v;
            }
        )
    );
    ...
    hpContainer.appendChild(
        createColor(
            "Ally Structures Color",
            window.RD_VISUALS.hpBuild.colorAlly,
            v=>{
                window.RD_VISUALS.hpBuild.colorAlly = v;
            }
        )
    );
    ...
    hpContainer.appendChild(
        createColor(
            "Enemy Structures Color",
            window.RD_VISUALS.hpBuild.colorEnemy,
            v=>{
                window.RD_VISUALS.hpBuild.colorEnemy = v;
            }
        )
    );
}
...
(function initUIModule() {
    ...
    uiBtn.className = 'rd-nav-btn';
    uiBtn.dataset.id = 'ui';
    uiBtn.textContent = 'UI';
    navBar.appendChild(uiBtn);

    const uiPage = document.createElement('div');
    uiPage.className = 'rd-page';
    uiPage.dataset.page = 'ui';
    uiPage.id = 'uiPage';
    uiPage.style.overflow = 'auto';
    content.appendChild(uiPage);

    uiBtn.addEventListener('click', () => {
        navBar.querySelectorAll('.rd-nav-btn').forEach(b=>b.classList.remove('active'));
        content.querySelectorAll('.rd-page').forEach(p=>p.classList.remove('active'));
        uiBtn.classList.add('active');
        uiPage.classList.add('active');
    });

    const wrapper = menu.querySelector('.rd-wrapper');
    let defaultTheme = {accent:'#c40000', bgMain:'#0a0a0a', bgPanel:'#1a1a1a'};
    let uiState = {
        blurEnabled: true,
        shadow: true,
        accentColor: defaultTheme.accent,
        bgMain: defaultTheme.bgMain,
        bgPanel: defaultTheme.bgPanel,
        borderRadius: 16
    };

    function applyUIChanges() {
        wrapper.style.background = uiState.bgMain;
        wrapper.style.borderRadius = uiState.borderRadius+'px';
        wrapper.style.boxShadow = uiState.shadow ? `0 0 40px ${uiState.accentColor}` : 'none';
        menu.style.backdropFilter = uiState.blurEnabled ? 'blur(6px)' : 'none';

        uiPage.querySelectorAll('.option-box').forEach(o=>{
            o.style.background = '#111';
            o.style.border = `2px solid ${uiState.accentColor}`;
        });

        const rangeInput = uiPage.querySelector('input[type="range"]');
        if(rangeInput){
            rangeInput.style.accentColor = uiState.accentColor;
            rangeInput.style.background = `linear-gradient(to right, ${uiState.accentColor} 0%, ${uiState.accentColor} ${(rangeInput.value-rangeInput.min)/(rangeInput.max-rangeInput.min)*100}%, #333 ${(rangeInput.value-rangeInput.min)/(rangeInput.max-rangeInput.min)*100}%, #333 100%)`;
        }

        const r = document.documentElement;
        r.style.setProperty('--accent', uiState.accentColor);
        r.style.setProperty('--bg-main', uiState.bgMain);
        r.style.setProperty('--bg-panel', uiState.bgPanel);
    }
    ...
    function optionBox(el) {
        ...
        box.style.border=`2px solid ${uiState.accentColor}`;
    ...
    uiPage.append(
        toggle('Enable Blur', uiState.blurEnabled, v=>{uiState.blurEnabled=v; applyUIChanges();}),
        toggle('Menu Shadow', uiState.shadow, v=>{uiState.shadow=v; applyUIChanges();})
    );

    function slider(label, min, max, value, cb){
        ...
        input.style.accentColor = uiState.accentColor;
        input.oninput=()=>{
            cb(parseFloat(input.value));
            applyUIChanges();
        };
        container.appendChild(input);
        return optionBox(container);
    }

    uiPage.append(
        slider('Menu Roundness',0,32,uiState.borderRadius,v=>{uiState.borderRadius=v; applyUIChanges();})
    );

    function colorPicker(label,value,cb){
        const container=document.createElement('div');
        container.style.display='flex';
        container.style.alignItems='center';
        container.style.justifyContent='space-between';
        const span=document.createElement('span'); span.textContent=label; span.style.color='#fff'; span.style.fontWeight='500';
        const input=document.createElement('input'); input.type='color'; input.value=value; input.oninput=()=>cb(input.value);
        container.appendChild(span); container.appendChild(input);
        return optionBox(container);
    }

    uiPage.append(
        colorPicker('Accent Color',uiState.accentColor,v=>{uiState.accentColor=v; applyUIChanges();}),
        colorPicker('Background Color',uiState.bgMain,v=>{uiState.bgMain=v; applyUIChanges();}),
        colorPicker('Panel Color',uiState.bgPanel,v=>{uiState.bgPanel=v; applyUIChanges();})
    );

const themes = [
    {name:'Red Dragon',accent:'#c40000',main:'#0a0a0a',panel:'#1a1a1a'},
    {name:'Blue Neon',accent:'#00aaff',main:'#050b14',panel:'#0b1c2d'},
    {name:'Purple Dark',accent:'#a855f7',main:'#0b0214',panel:'#160726'},
    {name:'Matrix',accent:'#00ff88',main:'#001a0d',panel:'#00331a'},
    {name:'Sunset Glow',accent:'#ff5500',main:'#140000',panel:'#2a0d0d'},
    {name:'Ice Frost',accent:'#00ffff',main:'#001b1b',panel:'#002828'},
    {name:'Cyberpunk',accent:'#ff00ff',main:'#1a001a',panel:'#2c002c'},
    {name:'Forest Mist',accent:'#00aa55',main:'#001a0f',panel:'#002814'},
    {name:'Solar Flare',accent:'#ffaa00',main:'#1a1400',panel:'#2c2200'},
    {name:'Midnight Blue',accent:'#0044ff',main:'#00051a',panel:'#000b2c'},
    {name:'Crimson Night',accent:'#aa0000',main:'#1a0000',panel:'#2c0000'},
    {name:'Neon Pulse',accent:'#ff00aa',main:'#1a001a',panel:'#2c002c'},
    {name:'Ocean Deep',accent:'#0055ff',main:'#000a1a',panel:'#00142c'},
    {name:'Desert Sand',accent:'#ffaa55',main:'#1a1400',panel:'#2c2200'},
    {name:'Violet Storm',accent:'#aa55ff',main:'#100014',panel:'#1c001f'},
    {name:'Golden Hour',accent:'#ffd700',main:'#1a1500',panel:'#2c2200'},
    {name:'Aurora',accent:'#55ffaa',main:'#001a10',panel:'#002814'},
    {name:'Cherry Blossom',accent:'#ff88aa',main:'#1a0014',panel:'#2c001f'},
    {name:'Steel Grey',accent:'#888888',main:'#101010',panel:'#1a1a1a'},
    {name:'Neon Rain',accent:'#00ffff',main:'#05050a',panel:'#0a0a14'},
    {name:'Emerald Night',accent:'#00cc66',main:'#001400',panel:'#002a10'},
    {name:'Ruby Twilight',accent:'#ff0055',main:'#1a000f',panel:'#2c001a'},
    {name:'Sapphire Depths',accent:'#0066ff',main:'#000a14',panel:'#001429'},
    {name:'Amber Dusk',accent:'#ffaa00',main:'#140a00',panel:'#2c1500'},
    {name:'Obsidian Void',accent:'#444444',main:'#000000',panel:'#111111'},
    {name:'Mint Breeze',accent:'#55ffaa',main:'#001410',panel:'#00281f'},
    {name:'Lavender Dream',accent:'#cc88ff',main:'#0a0014',panel:'#1a002a'},
    {name:'Copper Flame',accent:'#ff7700',main:'#140a00',panel:'#2c1500'},
    {name:'Galaxy Pulse',accent:'#ff55ff',main:'#0a0014',panel:'#1c0029'},
    {name:'Twilight Forest',accent:'#00aa55',main:'#00140a',panel:'#002819'},
    {name:'Electric Sunset',accent:'#ff3300',main:'#140000',panel:'#2c0a00'},
    {name:'Polar Night',accent:'#88ffff',main:'#000a0f',panel:'#00141f'},
    {name:'Rosewood',accent:'#ff3366',main:'#14000a',panel:'#2c0014'},
    {name:'Neon Jungle',accent:'#00ff88',main:'#001a0d',panel:'#00331a'},
    {name:'Cobalt Mist',accent:'#3366ff',main:'#000a14',panel:'#001429'},
    {name:'Golden Mirage',accent:'#ffd555',main:'#1a1400',panel:'#2c2200'},
    {name:'Crimson Aurora',accent:'#ff0055',main:'#10000a',panel:'#2c0014'},
    {name:'Electric Ocean',accent:'#00aaff',main:'#001014',panel:'#002028'},
    {name:'Midnight Pulse',accent:'#aa00ff',main:'#0a0014',panel:'#1a002a'},
    {name:'Plasma Wave',accent:'#ff0066',main:'#0a0014',panel:'#1a001f'},
    {name:'Iceberg Glow',accent:'#33ffff',main:'#001a1a',panel:'#002828'},
    {name:'Crimson Eclipse',accent:'#cc0000',main:'#14000a',panel:'#2a0014'},
    {name:'Obsidian Flame',accent:'#ff5500',main:'#0a0000',panel:'#1a0000'},
    {name:'Emerald Pulse',accent:'#00cc88',main:'#001a10',panel:'#00281a'},
    {name:'Violet Horizon',accent:'#aa33ff',main:'#0a0014',panel:'#1c0029'},
    {name:'Neon Frost',accent:'#00ffff',main:'#001417',panel:'#00282c'},
    {name:'Aurora Drift',accent:'#55ffaa',main:'#001010',panel:'#002420'},
    {name:'Ruby Wave',accent:'#ff0044',main:'#14000a',panel:'#2c0014'},
    {name:'Cobalt Spark',accent:'#3366ff',main:'#000a14',panel:'#001429'},
    {name:'Solar Ember',accent:'#ffaa33',main:'#140a00',panel:'#2c1500'},
    {name:'Midnight Mirage',accent:'#aa00ff',main:'#0a0014',panel:'#1a0029'},
    {name:'Electric Tide',accent:'#00aaff',main:'#001014',panel:'#002028'},
    {name:'Frozen Nebula',accent:'#88ffff',main:'#000a0f',panel:'#00141f'},
    {name:'Crimson Dawn',accent:'#ff3333',main:'#14000a',panel:'#2c0014'},
    {name:'Obsidian Night',accent:'#444444',main:'#000000',panel:'#111111'},
    {name:'Aurora Beam',accent:'#55ffaa',main:'#001410',panel:'#002828'},
    {name:'Sapphire Ember',accent:'#0066ff',main:'#000a14',panel:'#001429'},
    {name:'Lavender Pulse',accent:'#cc88ff',main:'#0a0014',panel:'#1a0029'},
    {name:'Golden Aurora',accent:'#ffd700',main:'#1a1400',panel:'#2c2200'}
];

    const themeSelect=document.createElement('select');
    themeSelect.className='rd-select';
    themeSelect.innerHTML=themes.map(t=>`<option value="${t.name}">${t.name}</option>`).join('');
    uiPage.append(optionBox(themeSelect));

    themeSelect.addEventListener('change',()=>{
        const t = themes.find(th=>th.name===themeSelect.value);
        if(t){
            uiState.accentColor=t.accent;
            uiState.bgMain=t.main;
            uiState.bgPanel=t.panel;
            applyUIChanges();
        }
    });
...
(function() {
    ...
    const headerImg = document.querySelector('#moddedMenu .rd-header img');

    if(headerImg) {
        headerImg.onerror = () => {
            headerImg.src = "https://i.imgur.com/vXgDJSp.png";
        };
