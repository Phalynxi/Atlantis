// ==UserScript==
// @name        Revenir Mod upgraded by CZGAMERSK
// @namespace    -
// @version     v1
// @description Revenir Mod the whole new private hack!
// @author      me mega noob :( upgraded revenir by CZGAMERSK
// @match        *://moomoo.io/*
// @match        *://sandbox.moomoo.io/*
// @grant        none
// @license MIT
// @downloadURL https://update.greasyfork.org/scripts/439784/Revenir%20Mod%20upgraded%20by%20CZGAMERSK.user.js
// @updateURL https://update.greasyfork.org/scripts/439784/Revenir%20Mod%20upgraded%20by%20CZGAMERSK.meta.js
// ==/UserScript==
...
, function(e, t, n) {
    ...
    Ne.width = 300,
    Ne.height = 300;
    Ie.replaceWith(document.createElement("a")),Ie.style.fontSize = "20px",Ie.style.display = "block",Ie.style.zIndex = "1";
    ...
    var at = document.getElementById("featuredYoutube")
      , ot = [{
        name: "CZGAMER SK",
        link: "https://www.youtube.com/channel/UC5AyIttB2zjI1wNoRaWRP1w"
    }]
    ...
    function pn() {
        if(AutoAim == false && intrap == false) {
            ...
            return nEA
        }else if(AutoAim == false && intrap == true) {
            return trapAngle
        }
    }
    ...
    function wasdMillsChangeFunction() {
    ...
    const booT = repe("f", () => {
        Sn(R.items[4]), r.send("c", 1, gIA(pn())), r.send("c", 0, gIA(pn())), Sn(R.weaponIndex, 1);
    }, 0);
    const SpiK = repe("v", () => {
        Sn(R.items[2]), r.send("c", 1, gIA(pn())), r.send("c", 0, gIA(pn())), Sn(R.weaponIndex, 1);
    }, 0);
    const TurR = repe("h", () => {
        Sn(R.items[5]), r.send("c", 1, gIA(pn())), r.send("c", 0, gIA(pn())), Sn(R.weaponIndex, 1);
    }, 0);
    window.addEventListener("keydown", s.checkTrusted((function(e) {
        var t = e.which || e.keyCode || 0;
        booT.start(e.key), SpiK.start(e.key), TurR.start(e.key);
        78 == t ? wasdMillsChangeFunction() : "";
    ...
    window.addEventListener("keyup", s.checkTrusted((function(e) {
        if (R && R.alive) {
            booT.stop(e.key), SpiK.stop(e.key), TurR.stop(e.key);
            var t = e.which || e.keyCode || 0;
    ...
    let trapAngle, clearedTeam = true;
    function isTeam(e) {
    ...
    setInterval(() => {
        if(R.alive) {
            if(R.team && Pt.length && clearedTeam == true) {
                clearedTeam = false;
            }
            if(!R.team && Pt.length && clearedTeam == false) {
                Pt = [];
                clearedTeam = true;
            }
        intrap = false;
            for(let i = 0; i < N.length; i++) {
                if(N[i].name == "pit trap" && N[i].owner.sid != R.sid && !isTeam(N[i].owner.sid)) {//autobreak
                    if(Math.hypot(N[i].y-R.y2, N[i].x-R.x2) <= 60 && N[i].active==!0 && AutoAim == false) {
                        trapAngle=Math.atan2((N[i].y - R.y2), (N[i].x - R.x2));
                        intrap = true;
                        if(R.weapons[1] == 10) {
                            if(R.weaponIndex != R.weapons[1]) {
                                r.send("5", R.weapons[1], 1);
                            }
                            r.send("13c", 0, (secondaryReload[R.sid]==1)?40:6, 0);
                        }else {
                            if(R.weaponIndex != R.weapons[0]) {
                                r.send("5", R.weapons[0], 1);
                            }
                            r.send("13c", 0, (primaryReload[R.sid]==1)?40:6, 0);
                        }
                        r.send("13c", 0, 21, 1);
                        r.send("c", 1, trapAngle), r.send("c", 0, trapAngle);
                        r.send("2", trapAngle);
                        r.send("2", trapAngle);
                    }
                }
            ...
            let EnY = Ii(nEy[0]);
            let rO = (EnY) ? (EnY.skinIndex == 11) ? 1.65 : 1.56 : 0.01;
            if(nEy && (fgd(nEy, R)/rO) < l.weapons[R.weapons[0]].range && AutoAim == false && intrap==false && secondaryReload[R.sid]==1) {
                if(R.tailIndex==11) {
                    Jt(0, 1);
                }
                Jt((primaryReload[R.sid]==1) ? (EnY.skinIndex == 11) ? 6 : 7 : 6, 0),
                Jt(21, 1);
                r.send("5", R.weapons[0], 1);
                r.send("c", 1, nEA), r.send("c", 0, nEA);
                r.send("2", nEA), r.send("2", nEA);
            }
            if(R.weaponIndex != 11) {
                if(intrap==false && secondaryReload[R.sid]==1 && AutoAim == false) {//auto reload ggez
                    if(R.weaponIndex != R.weapons[0]) {
                        r.send("5", R.weapons[0], 1);
                    }
                }else if(intrap==false && secondaryReload[R.sid]!=1 && AutoAim == false) {
                    if(R.weaponIndex != R.weapons[1]) {
                        r.send("5", R.weapons[1], 1);
                    }
                }
    ...
    function Cn(e) {
        for(let i = 0; i<N.length;i++) {
            if(N[i].sid == e) {
                if(AutoAim == false && Math.hypot(N[i].y-R.y2, N[i].x-R.x2) < 600) {
                    if(nEy && fgd(nEy, R) < 200) {
                        for(let i=0;i<12;i++) {
                            Sn(R.items[2]), r.send("c", 1, gIA(nEA+torad(i*30))), r.send("c", 0, gIA(nEA+torad(i*30))), Sn(R.weaponIndex, 1);
                            Sn(R.items[2]), r.send("c", 1, gIA(nEA-torad(i*30))), r.send("c", 0, gIA(nEA-torad(i*30))), Sn(R.weaponIndex, 1);
                        }
                    }else {
                        for(let i=0;i<12;i++) {
                            Sn(R.items[4]), r.send("c", 1, gIA(nEA+torad(i*30))), r.send("c", 0, gIA(nEA+torad(i*30))), Sn(R.weaponIndex, 1);
                            Sn(R.items[4]), r.send("c", 1, gIA(nEA-torad(i*30))), r.send("c", 0, gIA(nEA-torad(i*30))), Sn(R.weaponIndex, 1);
                        }
                    }
                }
    ...
    function On() {
        ...
        if(R.kills > Ue.innerText) {
            r.send("ch", "GG - Revenir upgraded");
        }
    ...
    function Yn(e, t, n) {
        for (var i, r, s, a = 0; a < N.length; ++a)
            ...
            be.save(),
            be.translate(r, s),
            (_.dir > 1e300 || _.dir < -1e300 ? (be.globalAlpha = 0.6, be.rotate(Math.atan2(Math.sin(_.dir), Math.cos(_.dir)))) : be.rotate(_.dir)),
    ...
    function Nn(e, t) {
        ...
        si(0, 0, e.scale, t),
               (t = t || be).beginPath(),
               t.fillStyle = "#ffffff00",
               t.strokeStyle = "#000000",
               t.arc(0, 0, l.weapons[e.weaponIndex].range+30, 0, 2*Math.PI),
               null || t.fill(),
               null || t.stroke(),
               t.globalAlpha = 1,
    ...
    var ii = [];
    function ri(e, t) {
        var n = ii[e.id + (R && e.owner && e.owner.sid == R.sid ? 0 : (R && R.team && e.owner && isTeam(e.owner.sid) ? 25 : 50))];
        if (!n || t) {
            ...
            t || (
                r.strokeStyle = (R && e.owner && e.owner.sid == R.sid ? "#8ecc51" : (R && R.team && e.owner && isTeam(e.owner.sid) ? "#9B870C" :"#cc5151")),
                r.globalAlpha = 1,
                si(0, 0, e.scale/8, r, 0, 1),
                r.fillStyle = (R && e.owner && e.owner.sid == R.sid ? "#8ecc51" : (R && R.team && e.owner && isTeam(e.owner.sid) ? "#9B870C" :"#cc5151")),
                r.fill()
                  ),
            n = i,
            t || (ii[e.id + (R && e.owner && e.owner.sid == R.sid ? 0 : (R && R.team && e.owner && isTeam(e.owner.sid) ? 25 : 50))] = n)
        }
        return n
    }
    ...
    function fi(e, t, n, i, r, s, a, o) {
        lt && (J.addProjectile(e, t, n, i, r, s, null, null, a).sid = o,setAdvCooldown(e, t, n, i, r))
    }
    function setAdvCooldown(e, t, n, i, r){
        ...
        if(Math.sqrt(min) > 60){
            if(r == 1.5){
                for(let i = 0; i < W.length;i++){
                    (_ = W[i]) && _.visible && min > (_.x2*1.5-_.x1/2-e+Math.cos(n)*10)**2+(_.y2*1.5-_.y1/2-t+Math.sin(n)*10)**2 && (id = _.sid, min = (_.x2*1.5-_.x1/2-e+Math.cos(n)*10)**2+(_.y2*1.5-_.y1/2-t+Math.sin(n)*10)**2)
                }
                if(Math.sqrt(min) < 60){
   ...
   function vi(e, t) {
    ...
    let autobuyinsta=setInterval(() => {
        autobuyhatsssssss();
    }, 500);
    var buybutbut=0;
    function autobuyhatsssssss() {
        if(Oe.innerText >= hatsacctobuy[buybutbut].cost) {
            if(hatsacctobuy[buybutbut].type == "acc") {
                r.send("13c", 1, hatsacctobuy[buybutbut].id, 1);
                hatsacctobuy[buybutbut].status=true;
                setTimeout(() => {
                    buybutbut++;
                }, 20);
            }else {
                r.send("13c", 1, hatsacctobuy[buybutbut].id, 0);
                hatsacctobuy[buybutbut].status=true;
                setTimeout(() => {
                    buybutbut++;
                }, 20);
        ...
        if(hatsacctobuy[11].status == true) {
            clearInterval(autobuyinsta);
        }
    }
    var AutoAim = false, iEn;
    function checkSecondaryInsta() {
        let enemyPlayer = (nEy)?Ii(nEy[0]):[];
        if(enemyPlayer.secondary != 10 && enemyPlayer.secondary != 11) {
            return true;
        }else {
            return false;
        }
    ...
    function Si(e, t) {
        ...
        let enemyPlayer = (nEy)?Ii(nEy[0]):[];
        if(R==_) {
            let damage = 100 - R.health;
            let healSecIfNes = (R.clownCounter >= 4) ? true : false;
            var healMS = (enemyPlayer) ? (enemyPlayer.primary) ? (enemyPlayer.primary == 7) ? 100 : 130 : 130 : 130;
            if(enemyPlayer) {
                if(SpamHIT == 0 && damage > 25 && damage >= l.weapons[(enemyPlayer.primary)?enemyPlayer.primary:0].dmg && secondaryReload[enemyPlayer.sid] == 1 && checkSecondaryInsta() == true) {
                    if(nEy && fgd(nEy, R) < 250 && enemyPlayer.primary != 0 && enemyPlayer.primary != 8) {
                        SpamHIT++;
                        setTimeout(() => {//weak anti-bull spam
                            SpamHIT--;
                        }, 550);
                        for(let i=0;i<4;i++) {
                            Sn(R.items[0]), r.send('c', 1, R.dir), r.send('c', 0, R.dir), Sn(R.weaponIndex, 1);
                        }
                    }else {
                        setTimeout(function() {
                            for(let i=0;i<4;i++) {
                    ...
                    if(secondaryReload[enemyPlayer.sid] != 1 && primaryReload[enemyPlayer.sid] == 1 && checkSecondaryInsta() == true) {//anti reversed insta
                        SpamHIT++;
                        setTimeout(() => {
                            SpamHIT--;
                        }, 550);
    ...
    setInterval(() => {
        if(AutoAim == !true && intrap == !true && wasdmills == false) {//auto place
            var enemyTrap = false, pits = 0, spike = 0;
            for(let i = 0; i < N.length; i++) {
                if(N[i].name == "pit trap") {
                    if(Math.hypot(N[i].y-R.y2, N[i].x - R.x2) < 150) {
                        pits++;
                    }
                    if(N[i].owner.sid == R.sid || isTeam(N[i].owner.sid)) {
                        if(nEy && Math.hypot(N[i].y-nEy[2], N[i].x - nEy[1]) < 70) {
                            enemyTrap = true;
                        }
                    }
                }else if(N[i].name == ("spikes" || "greater spikes" || "spinning spikes" || "poison spikes")) {
                    if(Math.hypot(N[i].y-R.y2, N[i].x - R.x2) < 150) {
                        spike++;
                    }
                }
            }
            let total = spike+pits;
            if(R.items[4]) {
                if(fgd(nEy, R) < 600 && enemyTrap == false && total < 4) {
                    for(let i=0;i<12;i++) {
                        Sn(R.items[4]), r.send("c", 1, gIA(torad(30*i))), r.send("c", 0, gIA(torad(30*i))), Sn(R.weaponIndex, 1);
                    }
                }
                if(fgd(nEy, R) < 200 && enemyTrap == true  && total < 4) {
                    for(let i=0;i<12;i++) {
                        Sn(R.items[2]), r.send("c", 1, gIA(torad(30*i))), r.send("c", 0, gIA(torad(30*i))), Sn(R.weaponIndex, 1);
                    }
                }
            }
        }
    ...
    function placemill() {
        if(wasdmills == false) {
            if(Math.sqrt(Math.pow((millY - R.y2), 2) + Math.pow((millX - R.x2), 2)) > 80) {
                millX = R.x;millY = R.y;
            }
        }else {
            if(Math.sqrt(Math.pow((millY - R.y2), 2) + Math.pow((millX - R.x2), 2)) > 80) {
                let angle = Math.atan2(millY - R.y, millX - R.x);
                Sn(R.items[3]), r.send("c", 1, gIA(angle + torad(0))), r.send("c", 0, angle + torad(0)), Sn(R.weaponIndex, 1);
                Sn(R.items[3]), r.send("c", 1, gIA(angle - torad(72))), r.send("c", 0, gIA(angle - torad(72))), Sn(R.weaponIndex, 1);
                Sn(R.items[3]), r.send("c", 1, gIA(angle + torad(72))), r.send("c", 0, gIA(angle - torad(72))), Sn(R.weaponIndex, 1);
                Sn(R.items[3]), r.send("c", 1, gIA(angle - torad(36))), r.send("c", 0, gIA(angle - torad(36))), Sn(R.weaponIndex, 1);
                Sn(R.items[3]), r.send("c", 1, gIA(angle + torad(36))), r.send("c", 0, gIA(angle + torad(36))), Sn(R.weaponIndex, 1);
                millX = R.x;millY = R.y;
            }
        }
    ...
    function Ti(e) {
        ...
        if(instaIsOn == true) {
            let enemyPlayer = (nEy) ? Ii(nEy[0]) : [];
            let rO = (enemyPlayer) ? (enemyPlayer.skinIndex == 11) ? 1.65 : 1.56 : 0.01;
            if(enemyPlayer) {
                if(nEy && (fgd(nEy, R)/rO) < l.weapons[R.weapons[0]].range && primaryReload[R.sid] == 1 && secondaryReload[R.sid] == 1 && turretReload[R.sid] == 1) {
                    instaIsOn = false;
                    AutoAim=true;
                    r.send("2", nEA);
                    r.send("13c", 0, (enemyPlayer.skinIndex == 6)? 7 : 6, 0);
                    if(R.tailIndex == 11) {r.send("13c", 0, 0, 1);}
                    r.send("5", R.weapons[0], 1);
                    r.send("7", 1);
                    setTimeout(() => {
                        r.send("2", nEA);
        ...
        canTick = false;
        if(nEy) {
            if(Math.sqrt(Math.pow((R.y2-nEy[2]), 2) + Math.pow((R.x2-nEy[1]), 2)) < 250) {
                iEn = true;
                if(intrap == false && AutoAim == false) {
                    if(R.skinIndex != 6 || R.tailIndex != 21) {
                        Jt(6, 0);
                        Jt(21, 1);
                    }
                }
            }else if(Math.sqrt(Math.pow((R.y2-nEy[2]), 2) + Math.pow((R.x2-nEy[1]), 2)) > 500) {
                canTick = true;
            }
        }
        if(!nEy) {
        ...
        if(AutoAim == false && iEn == false && intrap == false) {
            if(R.tailIndex != 11) {
                Jt(11, 1);
            }
            if (R.y2 < 2400){
                if(R.skinIndex != 15 && canTick == false) {
                    Jt(15, 0);
                }
            } else if (R.y2 > 6850 && R.y2 < 7550){
                if(R.skinIndex != 31 && canTick == false) {
                    Jt(31, 0);
                }
            } else {
                if(R.skinIndex != 12 && canTick == false) {
                    Jt(12, 0);
                }
            }
        }
        if(!iI && Qe.style.display == "none"){
            if(R.clownCounter > 0 && (tC - R.lastBull) % 9 == 0 || (R.lastBull && FT > 1)){
                FT++;
            }else if(nEy.length && fgd(nEy, R) < 300){
                if(O && Tb == 0 && oW == R.weapons[0]){
                    if(turretReload[R.sid] == 1 && ((nEy[9] == 7 && (tC - Ii(nEy[0]).lastBull) % 9 == 0) || nEy[9] == 45)){
                    }
                }
            }
        }
        ...
        function() {
            ...
            a = 0; a < W.length + Y.length; ++a)
                if ((_ = W[a] || Y[a - W.length]).visible && (10 != _.skinIndex || _ == R || _.team && _.team == R.team)) {
                    var w = (_.team ? "[" + _.team + "] " : "") + (_.name || "") + (_.isPlayer?" <"+_.clownCounter+">":"");
                    if ("" != w) {
                        ...
                        be.lineJoin = "round",
                        be.strokeText(w, _.x - f, _.y - d - _.scale - o.nameY + (150)),
                        be.fillText(w, _.x - f, _.y - d - _.scale - o.nameY + (150)),
                        _.isLeader && Rn.crown.isLoaded) {
                            var b = o.crownIconScale;
                            k = _.x - f - b / 2 - be.measureText(w).width / 2 - o.crownPad,
                            be.drawImage(Rn.crown, k, _.y - d - _.scale - o.nameY - b / 2 - 5 + (150), b, b)
                        }
                        1 == _.iconIndex && Rn.skull.isLoaded && (b = o.crownIconScale,
                        k = _.x - f - b / 2 + be.measureText(w).width / 2 + o.crownPad,
                        be.drawImage(Rn.skull, k, _.y - d - _.scale - o.nameY - b / 2 - 5 + (150), b, b))
                    }
                    if(instaIsOn == true && nEy) {
                        if(_.isPlayer && nEy[0] == _.sid) {
                            be.textAlign = "center",
                            be.fillStyle = "#cc5151",
                            be.lineJoin = "round",
                            be.font = "20px Hammersmith One",
                            be.lineWidth = 6,
                            be.strokeText("target",_.x - f,_.y - d),
                            be.fillText("target",_.x - f,_.y - d),
                            be.lineCap = "round",
                            be.strokeStyle = '#cc5151',
                            be.lineWidth = (oe/640)*2,
                            be.beginPath(),
                    ...
                    let playerWeapon = (_.weaponIndex<9)?(_.pr<primaryReload[_.sid]?_.pr+(primaryReload[_.sid]-_.pr)*Math.min(1,_.dt/111):primaryReload[_.sid]) : (_.sr<secondaryReload[_.sid]?_.sr+(secondaryReload[_.sid]-_.sr)*Math.min(1,_.dt/111):secondaryReload[_.sid]);
                    _.health > 0 && (o.healthBarWidth,
                                            be.fillStyle = rt,
                                            be.roundRect(_.x - f - o.healthBarWidth - o.healthBarPad, _.y - d + _.scale + o.nameY - (150), 2 * o.healthBarWidth + 2 * o.healthBarPad, 17, 8),
                                            be.fill(),
                                            be.fillStyle = _ == R || _.team && _.team == R.team ? "#8ecc51" : "#cc5151",
                                            be.roundRect(_.x - f - o.healthBarWidth, _.y - d + _.scale + o.nameY + o.healthBarPad - (150), 2 * o.healthBarWidth * (_.health / _.maxHealth), 17 - 2 * o.healthBarPad, 7),
                                            be.fill()),
                               _.isPlayer && ((o.healthBarWidth,
                                            be.fillStyle = rt,
                                            be.roundRect(_.x - f - o.healthBarWidth - o.healthBarPad, _.y - d + _.scale + o.nameY - (150 + 16), 2 * o.healthBarWidth + 2 * o.healthBarPad, 17, 8),
                                            be.fill(),
                                            be.fillStyle = "#a5974c",
                                            be.roundRect(_.x - f - o.healthBarWidth, _.y - d + _.scale + o.nameY + o.healthBarPad - (150 + 16), 2 * o.healthBarWidth * playerWeapon, 17 - 2 * o.healthBarPad, 7),
                                            be.fill()))
                }
            ...
            if (!this.socket) {
                ...
                try {
                    ...
                    this.socket.onclose = function(e) {
                        ...
                        window.onbeforeunload = null;document.body.remove();window.location.href = window.location.href;
                    }
                    ,
...
setInterval(() => {
    setTimeout(() => {
        document.getElementById('chatBox').placeholder = "💬now💬";
        setTimeout(() => {
            document.getElementById('chatBox').placeholder = "💬Go sub💬";
            setTimeout(() => {
                document.getElementById('chatBox').placeholder = "💬To💬";
                setTimeout(() => {
                    document.getElementById('chatBox').placeholder = "💬CZGAMERSK💬";
                }, 100);
            }, 100);
        }, 100);
    }, 100);
