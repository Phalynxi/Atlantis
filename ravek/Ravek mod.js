...
// ========== CUSTOM TEXTURE PACK SYSTEM (ported from Robotis.js) ==========
const PackTextures = {
  hats: {
...
function getTexture(id, type) {
  ...
  img.src = url;
  _textureCache.set(url, img);
  return img;
}

const _originalSrcDesc = Object.getOwnPropertyDescriptor(Image.prototype, "src");
Object.defineProperty(Image.prototype, "src", {
  get() { return _originalSrcDesc.get.call(this); },
  set(value) {
    try {
      const paths = {
      ...
      for (const [path, { type, prefix }] of Object.entries(paths)) {
        if (value.includes(path)) {
          ...
          if (type === "hat" && id && id.includes("_"))
            id = id.split("_")[0] + "_" + id.split("_")[1];
          const replacement = getTexture(id, type);
          if (replacement) value = replacement.src;
          break;
        }
      }
    } catch (e) { }
    return _originalSrcDesc.set.call(this, value);
  },
  configurable: true,
});
// Preload textures
...
!function (e) {
  ...
  // ========== SHORE PATH / COASTLINE SYSTEM (ported from Robotis.js) ==========
  function seePoint(point) {
    if (!j) return false;
    var deltaX = Math.abs(point.x - j.x) - point.scale;
    var deltaY = Math.abs(point.y - j.y) - point.scale;
    return deltaX <= (maxScreenWidth / 2) * 1.3 && deltaY <= (maxScreenHeight / 2) * 1.3;
  }
  ...
  function ShorePath(startX, startY, endX, distance, float) {
    ...
    this.generate = function () {
      for (var i = 1; i <= this.amountPaths; i += 1) {
        var offsetY = i % 2 === 0 ? this.distance : 0;
        var x = this.startX + this.distance * (i - 1);
        var randomOffsetY = Math.floor(Math.random() * 35) + 10;
        var y = this.startY + ((this.float === "down" ? offsetY : -offsetY) +
          (i % 2 === 0 ? (Math.random() < 0.55 ? randomOffsetY : -randomOffsetY) : 0));
        this.path.set(i, [x, y, offsetY, randomOffsetY]);
      }
    };

    this.render = function (color, xOffset, yOffset) {
      var path = Array.from(this.path.values());
      if (!j || !j.active || !j.alive) return;
      for (var i = 1; i < path.length; i++) {
        var oldPoint = path[i - 1];
        var currentPoint = path[i];
        if (!seePoint({ x: oldPoint[0], y: oldPoint[1], scale: 10 })) continue;
        if (!seePoint({ x: currentPoint[0], y: currentPoint[1], scale: 10 })) continue;

        var pointOffset = this.distance / 2;
        var sidePoint1 = [oldPoint[0] - pointOffset / 2, oldPoint[1] + (oldPoint[2] === 0 ? pointOffset : -pointOffset)];
        var sidePoint2 = [currentPoint[0] + pointOffset * 1.15, currentPoint[1] + pointOffset * 1.2];
        var sidePoint3 = [currentPoint[0] + pointOffset * 1.35, currentPoint[1] - pointOffset * 1.15];
        ...
        mainContext.beginPath();
        mainContext.moveTo(oldPoint[0] - xOffset, oldPoint[1] - yOffset);
        mainContext.bezierCurveTo(sidePoint1[0] - xOffset, sidePoint1[1] - yOffset, sidePoint2[0] - xOffset, sidePoint2[1] - yOffset, currentPoint[0] + (currentPoint[3] >= 10 ? 3.5 : 1) - xOffset, currentPoint[1] - yOffset);
        mainContext.fill();
        mainContext.beginPath();
        mainContext.moveTo(currentPoint[0] - xOffset, currentPoint[1] - yOffset);
        mainContext.bezierCurveTo(sidePoint2[0] - xOffset, sidePoint2[1] - yOffset, sidePoint3[0] - xOffset, sidePoint3[1] - yOffset, currentPoint[0] + this.distance * 2 - xOffset, this.startY - yOffset);
        mainContext.fill();
        mainContext.restore();
      }
    };
  ...
  var snowBiomeTop = h.snowBiomeTop || 2400;
  var snowPath = new ShorePath(-maxScreenWidth, snowBiomeTop - 1, h.mapScale + maxScreenWidth * 2, 50, "down");
  var desertPath = new ShorePath(-maxScreenWidth, h.mapScale - snowBiomeTop + 1, h.mapScale + maxScreenWidth * 2, 50, "up");
  // ========== END SHORE PATH SYSTEM ==========

  // ========== SNOWFLAKE SYSTEM (ported from Robotis.js) ==========
  var snowStyle = document.createElement("style");
  snowStyle.textContent = "@keyframes fall{0%{transform:translateY(-10vh);opacity:1}100%{transform:translateY(110vh);opacity:0}}.fast-fall{animation-duration:" + (Math.random() + 1) + "s!important}";
  document.head.appendChild(snowStyle);

  function createSnowflake() {
    var sf = document.createElement("div");
    sf.className = "snowflake";
    sf.style.cssText = "position:absolute;width:10px;height:10px;background:#fff;border-radius:50%;z-index:9998;opacity:" + Math.random() + ";left:" + (Math.random() * 100) + "vw;animation:fall " + (Math.random() * 3 + 2) + "s linear infinite;";
    sf.addEventListener("animationiteration", function () {
  ...
  snowflakeContainer.style.cssText = "position:absolute;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:9998;display:none;overflow:hidden;";
  document.body.appendChild(snowflakeContainer);
  for (var _si = 0; _si < 55; _si++) {
    var _sf = createSnowflake();
    if (Math.random() > 0.7) _sf.classList.add("fast-fall");
    snowflakeContainer.appendChild(_sf);
  }
  // ========== END SNOWFLAKE SYSTEM ==========

  // ========== NIGHT MODE SYSTEM (ported from Robotis.js) ==========
  var nightModeOverlay = document.createElement("div");
  nightModeOverlay.id = "nightMode";
  nightModeOverlay.style.cssText = "display:none;position:absolute;pointer-events:none;background-color:rgb(0,0,100);opacity:0;top:0;width:100%;height:100%;animation-duration:5s;animation-name:night2;z-index:999;";
  document.body.appendChild(nightModeOverlay);
  var nightStyle = document.createElement("style");
  nightStyle.textContent = "@keyframes night1{from{opacity:0}to{opacity:0.45}}@keyframes night2{from{opacity:0.45}to{opacity:0}}";
  ...
  var renderNightMode = function (ctx, delta, offsetX, offsetY) {
    ...
    if (nightModeState.overlay.opacity > 0) {
      if (nightModeState.overlay.opacity > 0.3 && starSpawnTimer > 300) {
        starSpawnTimer = 0;
        var starCount = u.randInt(3, 5);
        for (var i = 0; i < starCount; i++) {
          var scaleMultiplier = u.randFloat(1, 3);
          nightModeState.starField.push({
            scaleMultiplier: scaleMultiplier,
            posX: offsetX * scaleMultiplier + u.randFloat(0, maxScreenWidth),
            posY: offsetY * scaleMultiplier + u.randFloat(0, maxScreenHeight),
            size: u.randFloat(2, 6),
            transparency: 0,
            visible: true,
            brightening: true,
            color: "rgb(255, 255, " + u.randInt(0, 255) + ")"
          });
        }
      } else {
    ...
    nightModeState.starField = nightModeState.starField.filter(function (star) { return star.visible; });

    for (var s = 0; s < nightModeState.starField.length; s++) {
      var star = nightModeState.starField[s];
      if (star.brightening) {
      ...
      ctx.translate(star.posX - offsetX * star.scaleMultiplier, star.posY - offsetY * star.scaleMultiplier);
      ctx.scale(star.size, star.size);
      // Draw star shape
      ctx.beginPath();
      ctx.moveTo(0, -3);
      for (var k = 1; k < 8; k++) {
        var angle = (Math.PI / 4) * k;
        var radius = k % 2 === 0 ? 3 : 1.5;
  ...
  var nightState = 1;
  var nightStateTime = Date.now();

  function toggleDay_NightMode() {
    nightModeState.isEnabled = !nightModeState.isEnabled;
    if (nightModeState.isEnabled) {
      nightModeOverlay.style.animationName = "night1";
      nightModeOverlay.style.opacity = 0.45;
      nightModeOverlay.style.display = "block";
    } else {
      nightModeOverlay.style.animationName = "night2";
      nightModeOverlay.style.opacity = 0;
      setTimeout(function () { nightModeOverlay.style.display = "none"; }, 5000);
    }
  }

  var changeDays = setInterval(function () {
  ...
  window.toggleNight = function () {
    clearInterval(changeDays);
    changeDays = null;
    toggleDay_NightMode();
    nightState = !nightState;
    nightStateTime = Date.now();
  };
  // ========== END NIGHT MODE SYSTEM ==========

  // ========== HAT CHANGER & AUTOBUY (simplified from Robotis.js) ==========
  var _lastEquippedHat = 0;
  var _lastEquippedAcc = 0;
  var _autoBuyList = [7, 6, 22, 40, 11, 53, 12, 26, 56]; // Bull, Soldier, EMPHelm, Tank, Flipper, Musketeer, Spike Gear, Shame, Demolist
  var _autoBuyAccList = [21, 18, 19, 13, 11]; // Wings, Waist cape, Skull cape, Angel Wings, Shadow Wings
  var _autoBuyDone = {};

  function hatChanger() {
    if (!j || !j.alive) return;
    var hat = 6; // default: soldier
    // Find nearest enemy
    var nearDist = Infinity;
    for (var i = 0; i < Y.length; i++) {
      var pl = Y[i];
      if (pl.visible && pl.alive && pl != j && (!j.team || pl.team !== j.team)) {
        var dx = pl.x - j.x, dy = pl.y - j.y;
    ...
    if (nearDist <= 300) {
      hat = 6; // Soldier hat when enemy close
    } else if (j.y < (h.snowBiomeTop || 2400)) {
      hat = 15; // Winter hat in snow
    } else {
      hat = 12; // Boost hat default (speed)
    }

    if (_lastEquippedHat !== hat) {
      Zt(hat, 0);
      _lastEquippedHat = hat;
    }
  }
  ...
  function autoBuy() {
    ...
    var gold = j.points || 0;
    // Buy hats
    for (var i = 0; i < _autoBuyList.length; i++) {
      var hatId = _autoBuyList[i];
      if (!_autoBuyDone['h' + hatId]) {
        var hatInfo = hatsData.find(function (h) { return h.id === hatId; });
        if (hatInfo && gold >= hatInfo.price) {
          Qt(hatId, 0);
          _autoBuyDone['h' + hatId] = true;
        }
      }
    }
    // Buy accessories
    for (var a = 0; a < _autoBuyAccList.length; a++) {
      var accId = _autoBuyAccList[a];
      if (!_autoBuyDone['a' + accId]) {
        var accInfo = accessoriesData.find(function (ac) { return ac.id === accId; });
        if (accInfo && gold >= accInfo.price) {
          Qt(accId, 1);
          _autoBuyDone['a' + accId] = true;
        }
      }
    }
  }
  ...
  // ========== END HAT CHANGER & AUTOBUY ==========

  // Object pool for render entries to avoid GC pressure
  var _renderPool = new Array(2048);
  var _renderPoolIdx = 0;

  function updateGame() {
    _renderPoolIdx = 0; // Reset pool each frame
    ...
    // Snow biome particle check
    if (j && j.y < (h.snowBiomeTop || 2400)) {
      snowflakeContainer.style.display = "block";
      var _snowCount = snowflakeContainer.querySelectorAll('.snowflake').length;
      if (_snowCount < 55) {
        var _newSnow = createSnowflake();
        if (Math.random() > 0.7) _newSnow.classList.add("fast-fall");
        snowflakeContainer.appendChild(_newSnow);
      }
    } else {
      removeSnowflakes();
    ...
    for (var p in function (e, t, n) {
      ...
      for (var i = 0; i < W.length; ++i) if ((L = W[i]) && L.active) {
        ...
        if (isOnScreen(a, o, L.scale / (0 != L.type || L.isItem ? 1 : .3) + (L.blocker || 0))) {
          ...
          J[s] || (J[s] = []);
          // Reuse render entry from pool to avoid GC pressure
          var c = _renderPool[_renderPoolIdx] || (_renderPool[_renderPoolIdx] = { obj: null, x: 0, y: 0, trunkOnly: false });
          c.obj = L; c.x = a; c.y = o; c.trunkOnly = false;
          _renderPoolIdx++;
          if (J[s].push(c), "sapling" === L.name && L.isItem) {
            var l = L.onBridge ? 6 : 1;
            var c2 = _renderPool[_renderPoolIdx] || (_renderPool[_renderPoolIdx] = { obj: null, x: 0, y: 0, trunkOnly: false });
            c2.obj = L; c2.x = a; c2.y = o; c2.trunkOnly = true;
            _renderPoolIdx++;
            J[l].push(c2);
          }
          if (!L.isItem && 0 === L.type) {
            var c3 = _renderPool[_renderPoolIdx] || (_renderPool[_renderPoolIdx] = { obj: null, x: 0, y: 0, trunkOnly: false });
            c3.obj = L; c3.x = a; c3.y = o; c3.trunkOnly = true;
            _renderPoolIdx++;
            J[10].push(c3);
          }
        }
    ...
    // Render shore paths (coastlines)
    if (j && j.alive) {
      if (j.y < snowBiomeTop + maxScreenHeight) {
        snowPath.render("#fff", f, d);
      }
      if (j.y > h.mapScale - snowBiomeTop - maxScreenHeight) {
        desertPath.render("#b6db66", f, d);
      }
    }
    ...
    if (mainContext.stroke(), mainContext.globalAlpha = 1, mainContext.strokeStyle = "#525252", renderLayer(0, f, d), mainContext.globalAlpha = 1, mainContext.lineWidth = 5.5, renderProjectilesLayer(0, f, d), renderAILayer(f, d, 0), renderPlayersLayer(f, d, 0), renderLayer(1, f, d), renderLayer(2, f, d), renderLayer(3, f, d), renderLayer(4, f, d), mainContext.globalAlpha = 1, mainContext.lineWidth = 5.5, renderProjectilesLayer(1, f, d), renderAILayer(f, d, 1), renderPlayersLayer(f, d, 1), renderLayer(5, f, d), renderLayer(8, f, d), renderLayer(6, f, d), renderLayer(7, f, d), renderLayer(9, f, d), renderLayer(10, f, d), renderDeadPlayers(f, d, 0), renderDeadPlayers(f, d, 1), function (e, t) {
      // Simplified tree transparency: decrease alpha when player is nearby instead of cutout
      var n = J[11];
      if (!n || !n.length) return;
      var r = h.treeCutoutRadius || 300;
      for (var l = 0; l < n.length; l++) {
        var p = n[l];
        L = p.obj;
        var baseAlpha = L.hideFromEnemy ? .6 : 1;
        // Check distance to all team players for alpha reduction
        var nearPlayer = false;
        for (var pl = 0; pl < F.length; pl++) {
          var plr = F[pl];
          if (plr.visible && (plr == j || (j.team && plr.team === j.team))) {
            var dx = L.x - plr.x, dy = L.y - plr.y;
            var dist = Math.sqrt(dx * dx + dy * dy);
            if (dist < r) {
              nearPlayer = true;
              baseAlpha *= Math.max(0.25, dist / r);
              break;
            }
          }
        }
        ...
        mainContext.drawImage(g, p.x - g.width / 2, p.y - g.height / 2);
      }
    ...
    // Night mode stars rendering
    renderNightMode(mainContext, C, f, d);
  ...
  (function e() {
    ...
    C = R - z;
    z = R;
    // Frame skip: if delta is too large (>200ms), cap it to avoid physics jumps
    // and skip heavy rendering if we're falling behind
    if (C > 200) C = 200;
    updateGame();
    (function () {
      if (perfDisplay) {
